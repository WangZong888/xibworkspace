...
object SaveRepayDtlInfo {
  ...
  def save_repaydtl(contractArr: Array[String]): Unit = {
    ...
    try {
      ...
      //先将数据备份到轨迹表里
      val back_info = "insert into repay_detail_info_back (rpterm,cert_id,cint,aint,credit_id,order_risk_id,total_amount,actual_paydate,credit_risk_id,credit_apply_no,trans_req_no,capital,sell_credit_id,oint,id) select rpterm,cert_id,cint,aint,credit_id,order_risk_id,total_amount,actual_paydate,credit_risk_id,credit_apply_no,trans_req_no,capital,sell_credit_id,oint,id  from repay_detail_info"
      ...
      contractArr.foreach(x => {
        ...
        val capital = Global.getStringValue(lines(10)).getOrElse("0.0")
        val aint = Global.getStringValue(lines(11)).getOrElse("0.0")
        val oint = Global.getStringValue(lines(12)).getOrElse("0.0")
        val cint = Global.getStringValue(lines(13)).getOrElse("0.0")
