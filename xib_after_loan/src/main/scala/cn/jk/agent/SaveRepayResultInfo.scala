...
object SaveRepayResultInfo {
  ...
  def save_repayresult(contractArr: Array[String] ):Unit = {
    ...
    try{
      ...
      //先将数据备份到轨迹表里
      val back_info = "insert into repay_result_plan_info_back (rpterm,cert_id,ove_day,credit_id,total_amount,order_risk_id,actual_paydate,credit_risk_id,credit_apply_no,payment_accno,sell_credit_id,actual_capital,finish_date,pay_capital,actual_aint,actual_oint,actual_cint,expiry_date,pay_oint,status,value_date,rate,pay_date,pay_aint,pay_cint) select rpterm,cert_id,ove_day,credit_id,total_amount,order_risk_id,actual_paydate,credit_risk_id,credit_apply_no,payment_accno,sell_credit_id,actual_capital,finish_date,pay_capital,actual_aint,actual_oint,actual_cint,expiry_date,pay_oint,status,value_date,rate,pay_date,pay_aint,pay_cint  from repay_result_plan_info"
      ...
      contractArr.foreach(x=>{
       var map: mutable.Map[String, Object] = mutable.Map[String, Object]()
        ...
        val paymentAccno = lines(6)
        val rpterm = lines(7)
        val payDate = lines(8)
        val totalAmount = lines(9)
        val valueDate = lines(10)
        val expiryDate = lines(11)
        val rate = lines(12)
        val payCapital = lines(13)
        val actualCapital = lines(14)
        val payAint = lines(15)
        val actualAint = lines(16)
        val payOint = lines(17)
        val actualOint = lines(18)
        val payCint = lines(19)
        val actualCint = lines(20)
        val actualPaydate = lines(21)
        val finishDate = lines(22)
        val status = lines(23)
